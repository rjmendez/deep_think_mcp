...
kubectl create secret generic mqtt-credentials \
  --from-literal=host=[REDACTED_MQTT_HOST] \
...
kubectl create configmap deep-think-config \
  --from-literal=MQTT_BROKER_HOST=[REDACTED_MQTT_HOST] \
...
|----------|---------|----------|-------------|
| `MQTT_HOST` | `[REDACTED_MQTT_HOST]` | No | MQTT broker hostname |
...
mosquitto_sub -h [REDACTED_MQTT_HOST] -p 1883 -P password -t "dama/+/telemetry"
```
