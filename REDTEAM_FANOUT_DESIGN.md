...
```bash
# Start the async job
job_id=$(curl -X POST http://[REDACTED_INTERNAL_IP]:30830/think/fan-out \
...
while true; do
  status=$(curl -s http://[REDACTED_INTERNAL_IP]:30830/think/result/$job_id | jq -r '.status')
  if [ "$status" = "complete" ]; then
    curl -s http://[REDACTED_INTERNAL_IP]:30830/think/result/$job_id | jq .
    break
  fi
