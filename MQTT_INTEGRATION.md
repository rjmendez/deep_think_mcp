...
```
MQTT Broker (botnet.floppydicks.net)
    ↓
...
# Broker connection
MQTT_HOST=botnet.floppydicks.net         # Default: botnet.floppydicks.net
MQTT_PORT=1883                            # Default: 1883 (1883=plain, 8883=TLS)
MQTT_USERNAME=dama                        # Default: dama
MQTT_PASSWORD=A8YvoV9ML6wRl2VsiR4cp0t27Zap3hZZ  # From .env (secret)
...
>>> config = MQTTConfig()
>>> repr(config)
'MQTTConfig(host=botnet.floppydicks.net:1883, user=dama, enabled=True, batch_size=10, batch_timeout=5000ms)'
...
export MQTT_HOST=botnet.floppydicks.net
export MQTT_PORT=1883
export MQTT_USERNAME=dama
export MQTT_PASSWORD=A8YvoV9ML6wRl2VsiR4cp0t27Zap3hZZ
...
```bash
mosquitto_sub -h botnet.floppydicks.net -u dama -P <password> -t 'dama/+/telemetry' -v
```
